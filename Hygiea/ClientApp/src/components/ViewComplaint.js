...
class ViewComplaint extends React.Component {
	renderCheckBox(complaint) {
		if (GetTokenInfo().type === "student") {
			if (complaint.ResolvedByStaff) {
				return (
					<>
						<FormControl>
							<FormControlLabel
								control={
									<Checkbox
										checked={complaint.ResolvedByStudent}
										color="primary"
										onChange={() => this.props.toggleResolved(complaint._id)}
									/>
								}
								label="Tick if complaint has been resolved"
							/>
						</FormControl>
						{complaint.ResolvedByStudent ? <p>* Complaint Resolved</p> : <p>* waiting for your approval</p>}
					</>
				);
			}
		} else {
			if (complaint.ResolvedByStudent) return <p>* Complaint Resolved</p>;
			return (
				<>
					<FormControl>
						<FormControlLabel
							control={
								<Checkbox
									checked={complaint.ResolvedByStaff}
									color="primary"
									onChange={() => this.props.toggleResolved(complaint._id)}
								/>
							}
					...
					{!complaint.ResolvedByStaff ? null : <p>* waiting for complainant's approval</p>}
				</>
			);
		}
	}
	render() {
		...
		const accType = GetTokenInfo().type;
		return (
			<Modal className={modal_class} isOpen={this.props.open} backdrop={true}>
				...
				<ModalBody>
					...
					{this.props.complaint && !this.props.update ? this.renderCheckBox(this.props.complaint) : null}
				</ModalBody>
				<ModalFooter>
					{this.props.update ? (
